...
class ResNet(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        self.classifier_projection = nn.Sequential(nn.Linear(num_classes, num_classes, bias=False),
                                                   nn.BatchNorm1d(num_classes),
                                                   nn.ReLU(inplace=True),
                                                   nn.Linear(num_classes, num_classes, bias=False))
        self.ssl_linear = nn.Linear(nf * 8 * block.expansion, 1000)
    ...
    def forward(self, x: torch.Tensor, returnt="cls") -> Tuple:
        """
        Compute a forward pass.
        ...
        if returnt == "cls":
            ...
            return out_cls, F.normalize(out_ssl), F.normalize(self.classifier_projection(out_cls))
